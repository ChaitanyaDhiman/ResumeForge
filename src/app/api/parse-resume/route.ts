...
export async function POST(request: Request) {
  try {
    ...
    if (!session || !session.user) {
      return NextResponse.json({ error: 'Authentication required. Please sign in to use this feature.' }, { status: 401 });
    }

    // Fetch full user record to obtain user ID
    ...
    // Ensure email is verified before allowing optimization
    if (!userRecord.isEmailVerified) {
      return NextResponse.json({ error: 'Email not verified. Please verify your email before using this feature.' }, { status: 403 });
    }

    // 2. Rate limiting per user (using email)
    ...
    // 6. Sanitize job description to prevent XSS
    const sanitizedJobDescription = sanitizeText(jobDescription, 20000);

    // 7. Forward file to Flask parser
    ...
    try {
      ...
      return NextResponse.json({ error: `Invalid response from Python service (Status ${parserResponse.status}). Check server logs.` }, { status: 500 });
    }
    if (!parserResponse.ok) {
      return NextResponse.json({ error: `File parsing failed in Python service: ${errorData.error}` }, { status: parserResponse.status });
    }
    const { clean_text: resumeText } = errorData;
    const analysisPrompt = generateAnalysisPrompt(resumeText, sanitizedJobDescription);

    // 8. Call OpenAI for suggestions
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [
        { role: 'system', content: 'You are an expert JSON generator. You must only output the requested JSON object.' },
    ...
    try {
      ...
      return NextResponse.json({ error: 'LLM returned unparsable JSON. Try again.' }, { status: 500 });
    }

    // 9. Log successful optimization usage
    await prisma.optimizationLog.create({ data: { userId: userId } });
